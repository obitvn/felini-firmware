...
#define PIN_OUTPUT_ENABLE       (gpio_num_t )46
#define PIN_FUSB302_INT         (gpio_num_t )16
...
static void IRAM_ATTR fusb_tick_task(void) {
...
void PD_UFP_core_c::init_PPS(uint16_t PPS_voltage, uint8_t PPS_current, enum PD_power_option_t power_option)
{
    ...
    esp_register_freertos_tick_hook(fusb_tick_task);
...
FUSB302_ret_t PD_UFP_core_c::FUSB302_i2c_read(uint8_t dev_addr, uint8_t reg_addr, uint8_t *data, uint8_t count)
{
    ...
    esp_err_t res = i2c_manager_read(I2C_NUM_0, dev_addr, reg_addr,  data, count);
    return res == ESP_OK ? FUSB302_SUCCESS : FUSB302_ERR_READ_DEVICE;
}
...
FUSB302_ret_t PD_UFP_core_c::FUSB302_i2c_write(uint8_t dev_addr, uint8_t reg_addr, uint8_t *data, uint8_t count)
{
    ...
    esp_err_t res = i2c_manager_write(I2C_NUM_0, dev_addr, reg_addr,  data, count);
    return res == ESP_OK ? FUSB302_SUCCESS : FUSB302_ERR_READ_DEVICE;
}
    ...
    status_load_sw(0)
{
    ...
    gpio_set_direction(PIN_OUTPUT_ENABLE, GPIO_MODE_OUTPUT);
    gpio_set_pull_mode(PIN_OUTPUT_ENABLE, GPIO_PULLUP_PULLDOWN);
    gpio_set_level(PIN_OUTPUT_ENABLE, 0);
...
void PD_UFP_c::set_output(uint8_t enable)
{
    ...
    gpio_set_level(PIN_OUTPUT_ENABLE, enable);
...
int PD_UFP_log_c::status_log_readline(char * buffer, int maxlen)
{
    ...
    case STATUS_LOG_CC: {
        ...
        FUSB302_get_cc(&FUSB302, &cc1, &cc2);
        if (cc1 == 0 && cc2 == 0) {
            ESP_LOGI(TAG,"%sUSB attached vRA\n", t);
        } else if (cc1 && cc2 == 0) {
            ESP_LOGI(TAG,"%sUSB attached CC1 vRd-%s\n", t, detection_type_str[cc1 - 1]);
        } else if (cc2 && cc1 == 0) {
            ESP_LOGI(TAG,"%sUSB attached CC2 vRd-%s\n", t, detection_type_str[cc2 - 1]);
        } else {
            ESP_LOGI(TAG,"%sUSB attached unknown\n", t);
        }
        break; }
...
void PD_UFP_log_c::print_status(int serial)
{
...
void PD_UFP_log_c::print_status(uint16_t serial)
{
    // // Wait for enough tx buffer in serial port to avoid blocking
    // if (serial && serial.availableForWrite() >= SERIAL_BUFFER_SIZE - 1) {
        char buf[128];
