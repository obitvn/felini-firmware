...
struct rk816_battery {
	...
	uint8_t				ac_in;
	uint8_t				usb_in;
	uint8_t				otg_in;		/* OTG device attached status */
	uint8_t				otg_pmic5v;	/* OTG device power supply from PMIC */
	uint8_t				dc_in;
	uint8_t				prop_status;
	int				cvtlmt_irq;
	int				current_avg;
	...
	int				zero_dsoc;
	int				zero_linek;
	uint64_t				zero_drop_sec;
	uint64_t				shtd_drop_sec;
	int				sm_remain_cap;
	int				sm_linek;
	int				sm_chrg_dsoc;
	int				sm_dischrg_dsoc;
	...
	bool				is_force_calib;
	int				last_dsoc;
	uint8_t				cvtlmt_int_event;
	uint8_t				slp_dcdc_en_reg;
	int				ocv_pre_dsoc;
	int				ocv_new_dsoc;
	int				max_pre_dsoc;
	int				max_new_dsoc;
...
static uint64_t get_boot_sec(void)
{
	struct timespec ts;
...
static uint32_t interpolate(int value, uint32_t *table, int size)
{
	uint8_t i;
	uint16_t d;
...
/* (a*b)/c */
static int32_t ab_div_c(uint32_t a, uint32_t b, uint32_t c)
{
	bool sign;
	uint32_t ans = MAX_INT;
	int32_t tmp;
...
static int rk816_bat_read(struct rk816_battery *di, uint8_t reg)
{
	int ret, val;

	// ret = regmap_read(di->regmap, reg, &val);
    ret = i2c_manager_read(I2C_NUM_0, RK816_DEFAULT_I2C_ADDRESS, reg, &val, 1);
	if (ret)
		//ESP_LOGE(TAG, "read reg:0x%x failed\n", reg);

	return val;
}

static int rk816_bat_write(struct rk816_battery *di, uint8_t reg, uint8_t buf)
{
	// int ret;

	// ret = regmap_write(di->regmap, reg, buf);
	// if (ret)
	// 	//ESP_LOGE(TAG, "i2c write reg: 0x%2x error\n", reg);
	...
	// ret = regmap_read(di->regmap, reg, &val);
    ret = i2c_manager_write(I2C_NUM_0, RK816_DEFAULT_I2C_ADDRESS, reg, &val, 1);
	if (ret)
		//ESP_LOGE(TAG, "read reg:0x%x failed\n", reg);
...
static int rk816_bat_set_bits(struct rk816_battery *di, uint8_t reg, uint8_t mask, uint8_t buf)
{
	// ret = regmap_update_bits(di->regmap, reg, mask, buf);
	// if (ret)
	// 	//ESP_LOGE(TAG, "write reg:0x%x failed\n", reg);

	// return ret;

    uint8_t ret, orig, tmp;

    ret = i2c_manager_read(I2C_NUM_0, RK816_DEFAULT_I2C_ADDRESS, reg, &orig, 1);
    ...
    if (tmp != orig) 
    { 
        ret = i2c_manager_write(I2C_NUM_0, RK816_DEFAULT_I2C_ADDRESS, reg, &tmp, 1);
        if (ret)
            //ESP_LOGE(TAG, "read reg:0x%x failed\n", reg);
    }
...
static int rk816_bat_clear_bits(struct rk816_battery *di, uint8_t reg, uint8_t mask)
{
    uint8_t ret, orig, tmp;
...
static void rk816_bat_dump_regs(struct rk816_battery *di, uint8_t start, uint8_t end)
{
	int i;
	...
	//ESP_LOGD(TAG,"dump regs from: 0x%x-->0x%x\n", start, end);
	for (i = start; i < end; i++)
		//ESP_LOGD(TAG,"0x%x: 0x%0x\n", i, rk816_bat_read(di, i));
}
...
// 	if (ret != 1) {
// 		//ESP_LOGE(TAG, "error! cmd require only one args\n");
// 		return count;
// 	}
...
static void rk816_bat_save_age_level(struct rk816_battery *di, uint8_t level)
{
	rk816_bat_write(di, RK816_UPDATE_LEVE_REG, level);
}

static uint8_t rk816_bat_get_age_level(struct  rk816_battery *di)
{
...
static void rk816_bat_init_voltage_kb(struct rk816_battery *di)
{
	...
	//ESP_LOGD(TAG,"voltage_k=%d(*1000),voltage_b=%d\n", di->voltage_k, di->voltage_b);
}
...
static uint16_t rk816_bat_get_relax_vol1(struct rk816_battery *di)
{
	uint16_t vol, val = 0;
...
static uint16_t rk816_bat_get_relax_vol2(struct rk816_battery *di)
{
	uint16_t vol, val = 0;
...
static uint16_t rk816_bat_get_relax_voltage(struct rk816_battery *di)
{
...
static bool rk816_bat_adc_calib(struct rk816_battery *di)
{
	...
	for (i = 0; i < 5; i++) {
		if (!rk816_bat_chrg_online(di)) {
			...
			//ESP_LOGI(TAG,"quit, charger plugout when calib adc\n");
			return false;
		}
		...
		if (is_rk816_bat_st_cvtlim(di)) {
			rk816_bat_set_coffset(di, save_coffset);
			//ESP_LOGI(TAG,"1 cvtlmt(st) when calib adc\n");
			return false;
		}
		...
		if (di->cvtlmt_int_event) {
			...
			rk816_bat_set_coffset(di, save_coffset);
			//ESP_LOGI(TAG,"1 cvtlmt(int) when calib adc\n");
			return false;
		}
		...
		if (is_rk816_bat_st_cvtlim(di)) {
			rk816_bat_set_coffset(di, save_coffset);
			//ESP_LOGI(TAG,"2 cvtlmt(st) when calib adc\n");
			return false;
		}
		enable_irq(di->cvtlmt_irq);
		msleep(2000);
		...
		if (di->cvtlmt_int_event) {
			di->cvtlmt_int_event = 0;
			rk816_bat_set_coffset(di, save_coffset);
			//ESP_LOGI(TAG,"2 cvtlmt(int) when calib adc\n");
			return false;
		}
		...
		if (abs(adc) < ADC_CALIB_THRESHOLD) {
			...
			//ESP_LOGI(TAG,"new offset:c=0x%x, i=0x%x, p=0x%x\n",
				 coffset, ioffset, di->poffset);
			return true;
		} else {
			//ESP_LOGI(TAG,"coffset calib again %d.., max_cnt=%d\n",
				 i, di->adc_calib_cnt);
...
static void rk816_bat_set_relax_sample(struct rk816_battery *di)
{
	...
	//ESP_LOGD(TAG,"<%s>. sleep_enter_current = %d, sleep_exit_current = %d\n",
...
static void rk816_bat_lowpwr_check(struct rk816_battery *di)
{
	...
	if (di->current_avg < 0 && di->voltage_avg < pwr_off_thresd) {
		...
		if ((base2sec(time) > MINUTE(1)) ||
			...
			if (di->voltage_avg <= pwr_off_thresd - 50)
				di->dsoc--;
			//ESP_LOGI(TAG,"low power, soc=%d, current=%d\n",
	...
	//ESP_LOGD(TAG,"<%s>. t=%lu, dsoc=%d, current=%d, fake_offline=%d\n",
...
static uint8_t rk816_bat_get_pwroff_min(struct rk816_battery *di)
{
	uint8_t now_min, last_min;
...
static uint8_t is_rk816_bat_initialized(struct rk816_battery *di)
{
	uint8_t val = rk816_bat_read(di, RK816_MISC_MARK_REG);
...
static void rk816_bat_init_age_algorithm(struct rk816_battery *di)
{
	...
	if (di->bat_first_power_on || is_rk816_bat_ocv_valid(di)) {
		...
		if (ocv_soc < 20) {
			...
			//ESP_LOGI(TAG,"init_age_algorithm: age_vol:%d, age_ocv_cap:%d, age_ocv_soc:%d, old_age_level:%d, age_allow_update:%d, new_age_level:%d\n",
...
static int rk816_bat_init_power_supply(struct rk816_battery *di)
{
	...
	if (IS_ERR(di->bat)) {
		//ESP_LOGE(TAG, "register bat power supply fail\n");
	...
	if (IS_ERR(di->ac)) {
		//ESP_LOGE(TAG, "register ac power supply fail\n");
	...
	if (IS_ERR(di->usb)) {
		//ESP_LOGE(TAG, "register usb power supply fail\n");
...
static void rk816_bat_save_fcc(struct rk816_battery *di, uint32_t fcc)
{
	uint8_t buf;
...
static int rk816_bat_get_lock_fcc(struct rk816_battery *di)
{
	...
	//ESP_LOGI(TAG,"lock fcc = %d\n", fcc);

	return fcc;
}

static void rk816_bat_save_dsoc(struct rk816_battery *di, uint8_t save_soc)
{
...
static void rk816_bat_save_reboot_cnt(struct rk816_battery *di, uint8_t save_cnt)
{
...
static void rk816_bat_set_current(struct rk816_battery *di, int charge_current)
{
	...
	if (di->pdata->bat_mode == MODE_VIRTUAL) {
		//ESP_LOGI(TAG,"virtual power test mode, set max input current\n");
...
{
	uint8_t buf, usb_ctrl, chrg_ctrl1;
	...
	//ESP_LOGI(TAG,"set charger type: %s, current: input=%d, chrg=%d\n",
...
static void rk816_bat_set_otg_power(struct rk816_battery *di, int power)
{
	...
	case USB_OTG_POWER_ON:
		if (di->otg_pmic5v) {
			//ESP_LOGI(TAG,"otg5v is on yet, ignore..\n");
			break;
		}
		...
		buf = rk816_bat_read(di, RK816_VB_MON_REG);
		if (buf & PLUG_IN_STS) {
			//ESP_LOGI(TAG,"detect vbus-5v suppling, deny otg on..\n");
			break;
		}
		...
		if (buf & PLUG_IN_STS) {
			...
			//ESP_LOGI(TAG,"detect vbus-5v suppling too, deny otg on\n");
			break;
		}
	...
	case USB_OTG_POWER_OFF:
		if (!di->otg_pmic5v) {
			//ESP_LOGI(TAG,"otg5v is off yet, ignore..\n");
		} else {
			rk816_bat_set_bits(di, RK816_DCDC_EN_REG2,
					   BOOST_OTG_MASK, BOOST_OTG_OFF);
			di->otg_pmic5v = 0;
...
static void rk816_bat_init_coulomb_cap(struct rk816_battery *di, uint32_t capacity)
{
	uint8_t buf;
	uint32_t cap;
...
static uint8_t rk816_bat_get_halt_cnt(struct rk816_battery *di)
{
...
static void rk816_bat_first_pwron(struct rk816_battery *di)
{
	...
	//ESP_LOGI(TAG,"first on: dsoc=%d, rsoc=%d cap=%d, fcc=%d, ov=%d\n",
...
static void rk816_bat_not_first_pwron(struct rk816_battery *di)
{
	...
	if (di->is_initialized) {
		//ESP_LOGI(TAG,"initialized yet..\n");
		goto finish;
	} else if (di->is_halt) {
		//ESP_LOGI(TAG,"system halt last time... cap: pre=%d, now=%d\n",
			 pre_cap, now_cap);
		if (now_cap < 0)
			now_cap = 0;
		...
		if (abs(ocv_soc - pre_soc) >= di->pdata->max_soc_offset) {
			...
			//ESP_LOGI(TAG,"trigger max soc offset, dsoc: %d -> %d\n",
				 pre_soc, ocv_soc);
			pre_soc = ocv_soc;
		}
		//ESP_LOGI(TAG,"OCV calib: cap=%d, rsoc=%d\n", ocv_cap, ocv_soc);
		...
		if (abs(ocv_soc - pre_soc) >= 80) {
			...
			//ESP_LOGI(TAG,"dsoc force calib: %d -> %d\n",
				 pre_soc, ocv_soc);
			pre_soc = ocv_soc;
			pre_cap = ocv_cap;
		}
...
finish:
	...
	//ESP_LOGI(TAG,"dsoc=%d cap=%d v=%d ov=%d rv=%d min=%d psoc=%d pcap=%d\n",
...
static bool rk816_bat_ocv_sw_reset(struct rk816_battery *di)
{
	...
	if (((buf & FG_RESET_LATE) && di->pwroff_min >= 30) ||
		...
		//ESP_LOGI(TAG,"manual reset fuel gauge\n");
		return true;
	} else {
		return false;
	}
...
static void rk816_bat_setup_ocv_table(struct rk816_battery *di, int temp)
{
	...
	//ESP_LOGD(TAG,"<%s>. temperature=%d\n", __func__, temp);

	/* Out of MIN, select MIN */
	if (temp < temp_t[0]) {
		//ESP_LOGD(TAG,"<%s>. Out MIN\n", __func__);
	...
	for (i = 0; i < di->pdata->ocv_size; i++) {
		...
		//ESP_LOGD(TAG,"#low=%d'C[%dmv], me=%d'C[%dmv], high=%d'C[%dmv]. percent=%d, delta=%dmv\n",
...
static uint8_t rk816_bat_get_chrg_status(struct rk816_battery *di)
{
	uint8_t status;
	...
	switch (status) {
	case CHARGE_OFF:
		//ESP_LOGD(TAG,"CHARGE-OFF ...\n");
		break;
	case DEAD_CHARGE:
		//ESP_LOGI(TAG,"DEAD CHARGE...\n");
		break;
	case TRICKLE_CHARGE:
		//ESP_LOGI(TAG,"TRICKLE CHARGE...\n ");
		break;
	case CC_OR_CV:
		//ESP_LOGD(TAG,"CC or CV...\n");
		break;
	case CHARGE_FINISH:
		//ESP_LOGD(TAG,"CHARGE FINISH...\n");
		break;
	case USB_OVER_VOL:
		//ESP_LOGI(TAG,"USB OVER VOL...\n");
		break;
	case BAT_TMP_ERR:
		//ESP_LOGI(TAG,"BAT TMP ERROR...\n");
		break;
	case TIMER_ERR:
		//ESP_LOGI(TAG,"TIMER ERROR...\n");
		break;
	case USB_EXIST:
		//ESP_LOGI(TAG,"USB EXIST...\n");
		break;
	case USB_EFF:
		//ESP_LOGI(TAG,"USB EFF...\n");
		break;
	default:
...
static uint8_t rk816_bat_fb_temp(struct rk816_battery *di)
{
	uint8_t reg;
	int index, fb_temp;
...
static void rk816_bat_select_chrg_cv(struct rk816_battery *di)
{
	...
	//ESP_LOGD(TAG,"<%s>. vol = 0x%x, input = 0x%x, sel = 0x%x\n",
	    __func__, di->chrg_vol_sel, di->chrg_cur_input, di->chrg_cur_sel);
}

static uint8_t rk816_bat_finish_ma(struct rk816_battery *di, int fcc)
{
	uint8_t ma;
...
{
	uint8_t chrg_ctrl1, usb_ctrl, chrg_ctrl2, chrg_ctrl3;
	uint8_t sup_sts, thermal, ggcon, finish_ma, fb_temp;
...
static void rk816_bat_internal_calib(struct work_struct *work)
{
	...
	//ESP_LOGI(TAG,"caltimer: ioffset=0x%x, coffset=0x%x\n",
...
static void rk816_bat_init_zero_table(struct rk816_battery *di)
{
	...
	if (!di->pdata->zero_table) {
		...
		//ESP_LOGE(TAG, "malloc zero table fail\n");
		return;
	}
...
static void rk816_bat_calc_sm_linek(struct rk816_battery *di)
{
	...
	//ESP_LOGD(TAG,"<%s>.diff=%d, k=%d, cur=%d\n", __func__, diff, linek, current_avg);
}
...
static void rk816_bat_calc_zero_linek(struct rk816_battery *di)
{
	...
	//ESP_LOGD(TAG,"ZERO0: shtd_vol: org = %d, now = %d, zero_reserve_dsoc = %d\n",
	...
	//ESP_LOGD(TAG,"ZERO0: dead_voltage(shtd) = %d, ocv_voltage(now) = %d\n",
	...
	if ((di->dsoc <= 1) && (xsoc > 0)) {
		...
		if ((di->pdata->energy_mode) &&
			...
			di->zero_linek = 300;
			//ESP_LOGD(TAG,"ZERO-new: zero_linek adjust step0...\n");
		...
		/* dsoc[5~15], dsoc < xsoc */
			...
			/* reserve power */
				...
				di->zero_linek = 1200;
			//ESP_LOGD(TAG,"ZERO-new: zero_linek adjust step4...\n");
		/* dsoc[5, 100], dsoc < xsoc */
			...
			//ESP_LOGD(TAG,"ZERO-new: zero_linek adjust step5...\n");
		/* dsoc[0~5], dsoc < xsoc */
		} else if ((di->zero_linek < 1000) && (di->dsoc <= 5)) {
			if ((xsoc - di->dsoc) <= 3)
				di->zero_linek = 1200;
				...
				di->zero_linek = 800;
			//ESP_LOGD(TAG,"ZERO-new: zero_linek adjust step6...\n");
		}
	} else {
		/* xsoc < 0 */
		di->zero_linek = 1000;
	...
	if (voltage_avg < pwroff_vol - 70) {
		...
		if (base2sec(di->shtd_drop_sec) > WAIT_SHTD_DROP_SEC) {
			//ESP_LOGI(TAG,"voltage extreme low...soc:%d->0\n", di->dsoc);
			di->shtd_drop_sec = 0;
			di->dsoc = 0;
		}
	} else {
	...
	//ESP_LOGD(TAG,"ZERO-new: org_linek=%d, zero_linek=%d, dsoc=%d, Xsoc=%d, rsoc=%d, gap=%d, v=%d, vsys=%d\n"
...
static void rk816_bat_smooth_algo_prepare(struct rk816_battery *di)
{
	...
	//ESP_LOGD(TAG,"<%s>. tmp_soc=%d, dsoc=%d, dsoc:sm_dischrg=%d, sm_chrg=%d\n",
...
static void rk816_bat_zero_algo_prepare(struct rk816_battery *di)
{
	...
	//ESP_LOGD(TAG,"<%s>. first calc, reinit linek\n", __func__);
...
static void rk816_bat_calc_zero_algorithm(struct rk816_battery *di)
{
	...
	//ESP_LOGD(TAG,"<%s>. enter: dsoc=%d, rsoc=%d\n", __func__, di->dsoc, di->rsoc);
	...
	if (di->dsoc < di->rsoc) {
		...
		if (tmp_soc == di->dsoc) {
			...
			//ESP_LOGD(TAG,"ZERO1: take sm chrg,delta=%d\n", sm_delta_dsoc);
		}
	}
	...
	if (di->dsoc > di->rsoc) {
		...
		if (tmp_soc == di->dsoc) {
			...
			di->sm_dischrg_dsoc = (di->dsoc + 1) * 1000 -
								MIN_ACCURACY;
			di->zero_dsoc += sm_delta_dsoc;
			//ESP_LOGD(TAG,"ZERO1: take sm dischrg,delta=%d\n", sm_delta_dsoc);
		}
	}

	/* check overflow */
	if (di->zero_dsoc > (di->dsoc + 1) * 1000 - MIN_ACCURACY) {
		//ESP_LOGD(TAG,"ZERO1: zero dsoc overflow: %d\n", di->zero_dsoc);
		di->zero_dsoc = (di->dsoc + 1) * 1000 - MIN_ACCURACY;
	}
...
out:
	//ESP_LOGD(TAG,"ZERO1: zero_dsoc(Y0)=%d, dsoc=%d, rsoc=%d, tmp_soc=%d\n",
	    // di->zero_dsoc, di->dsoc, di->rsoc, tmp_soc);
	//ESP_LOGD(TAG,"ZERO1: sm_dischrg_dsoc=%d, sm_chrg_dsoc=%d\n",
...
static void rk816_bat_zero_algorithm(struct rk816_battery *di)
{
	...
	//ESP_LOGD(TAG,"ZERO1: zero_linek=%d, zero_dsoc(Y0)=%d, dsoc=%d, rsoc=%d\n"
...
static void rk816_bat_dump_time_table(struct rk816_battery *di)
{
	...
	for (i = 1; i < 11; i++)
		//ESP_LOGD(TAG,"Time[%d]=%d, ", (i * 10), di->dbg_chrg_min[i - 1]);
	//ESP_LOGD(TAG,"\n");
}

static void rk816_bat_debug_info(struct rk816_battery *di)
{
	uint8_t sup_tst, ggcon, ggsts, vb_mod, ts_ctrl, reboot_cnt;
	uint8_t usb_ctrl, chrg_ctrl1, thermal;
	uint8_t int_sts1, int_sts2, int_sts3;
	uint8_t int_msk1, int_msk2, int_msk3;
	uint8_t chrg_ctrl2, chrg_ctrl3, rtc, misc, dcdc_en2;
	uint32_t chrg_sel;
...
static void rk816_bat_init_capacity(struct rk816_battery *di, uint32_t cap)
{
	int delta_cap;
...
static void rk816_bat_update_age_fcc(struct rk816_battery *di)
{
	...
	//ESP_LOGD(TAG,"%s: lock_fcc=%d, age_ocv_cap=%d, age_adjust_cap=%d, remain_cap=%d, age_allow_update=%d, age_keep_min=%d\n",
...
static void rk816_bat_finish_algorithm(struct rk816_battery *di)
{
	...
	if (di->dsoc < 100) {
		...
		//ESP_LOGD(TAG,"<%s>.CHARGE_FINISH:dsoc<100,dsoc=%d\n"
...
static void rk816_bat_calc_smooth_dischrg(struct rk816_battery *di)
{
	...
	if (di->dsoc < di->rsoc) {
		...
		if (tmp_soc == di->dsoc) {
			...
			//ESP_LOGD(TAG,"<%s>. take sm dischrg, delta=%d\n",
	...
	if (di->dsoc > di->rsoc) {
		...
		if (tmp_soc == di->dsoc) {
			...
			//ESP_LOGD(TAG,"<%s>. take zero schrg, delta=%d\n",
	...
	if ((di->sm_dischrg_dsoc) > ((di->dsoc + 1) * 1000 - MIN_ACCURACY)) {
		//ESP_LOGD(TAG,"<%s>. dischrg_dsoc up overflow\n", __func__);
...
out:
	//ESP_LOGD(TAG,"<%s>. dsoc=%d, rsoc=%d, dsoc:sm_dischrg=%d, sm_chrg=%d, zero=%d\n",
...
static void rk816_bat_calc_smooth_chrg(struct rk816_battery *di)
{
	...
	if (di->dsoc > di->rsoc) {
		...
		if (tmp_soc == di->dsoc) {
			...
			//ESP_LOGD(TAG,"<%s>. take zero dischrg, delta=%d\n",
			    __func__, zero_delta_dsoc);
		}
	}
	...
	if (di->sm_chrg_dsoc < di->dsoc * 1000) {
		//ESP_LOGD(TAG,"<%s>. chrg_dsoc down overflow\n", __func__);
		di->sm_chrg_dsoc = di->dsoc * 1000;
	}
...
static void rk816_bat_smooth_algorithm(struct rk816_battery *di)
{
	...
	/* simulate charge, speed up */
		...
		if ((di->sm_linek * di->current_avg <= 0) ||
		    ...
		    (di->sm_linek == FULL_CHRG_K) ||
		    (di->sm_linek == SIMULATE_CHRG_K)) {
			//ESP_LOGD(TAG,"<%s>. linek mode, retinit sm linek..\n", __func__);
			rk816_bat_calc_sm_linek(di);
		}
	}
	...
	if ((di->dsoc == di->rsoc) && (abs(di->sm_linek) != 1000) &&
		...
		//ESP_LOGD(TAG,"<%s>. flat_sec=%ld, tgt_sec=%ld, sm_k=%d\n", __func__,
	...
	if ((abs(di->sm_linek) == 1000) || (di->current_avg >= 0 &&
		...
		//ESP_LOGD(TAG,"<%s>. sm_linek=%d\n", __func__, di->sm_linek);
		if (abs(di->sm_linek) == 1000) {
			di->dsoc = di->rsoc;
			di->sm_linek = (di->sm_linek > 0) ? 1000 : -1000;
			//ESP_LOGD(TAG,"<%s>. dsoc == rsoc, sm_linek=%d\n",
			    __func__, di->sm_linek);
		}
		di->sm_remain_cap = di->remain_cap;
		di->sm_chrg_dsoc = di->dsoc * 1000;
		di->sm_dischrg_dsoc = (di->dsoc + 1) * 1000 - MIN_ACCURACY;
		//ESP_LOGD(TAG,"<%s>. sm_dischrg_dsoc=%d, sm_chrg_dsoc=%d\n",
		    __func__, di->sm_dischrg_dsoc, di->sm_chrg_dsoc);
	} else {
		delta_cap = di->remain_cap - di->sm_remain_cap;
		if (delta_cap == 0) {
			//ESP_LOGD(TAG,"<%s>. delta_cap = 0\n", __func__);
			return;
		}
		ydsoc = di->sm_linek * abs(delta_cap) * 100 / DIV(di->fcc);
		if (ydsoc == 0) {
			//ESP_LOGD(TAG,"<%s>. ydsoc = 0\n", __func__);
			return;
		}
		di->sm_remain_cap = di->remain_cap;

		//ESP_LOGD(TAG,"<%s>. k=%d, ydsoc=%d; cap:old=%d, new:%d; delta_cap=%d\n",
...
static void rk816_bat_display_smooth(struct rk816_battery *di)
{
	...
	if (di->s2r && !di->sleep_chrg_online) {
		//ESP_LOGD(TAG,"s2r: discharge, reset algorithm...\n");
		di->s2r = false;
		rk816_bat_zero_algo_prepare(di);
		rk816_bat_smooth_algo_prepare(di);
		return;
	...
	if (di->work_mode == MODE_FINISH) {
		//ESP_LOGD(TAG,"step1: charge finish...\n");
		...
		if ((rk816_bat_get_chrg_status(di) != CHARGE_FINISH) &&
			...
			if ((di->current_avg < 0) &&
				...
				//ESP_LOGD(TAG,"step1: change to zero mode...\n");
				rk816_bat_zero_algo_prepare(di);
				di->work_mode = MODE_ZERO;
			} else {
				//ESP_LOGD(TAG,"step1: change to smooth mode...\n");
				rk816_bat_smooth_algo_prepare(di);
				di->work_mode = MODE_SMOOTH;
			}
		}
	} else if (di->work_mode == MODE_ZERO) {
		//ESP_LOGD(TAG,"step2: zero algorithm...\n");
		rk816_bat_zero_algorithm(di);
		if ((di->voltage_avg >= di->pdata->zero_algorithm_vol + 50) ||
		    (di->current_avg >= 0)) {
			//ESP_LOGD(TAG,"step2: change to smooth mode...\n");
			rk816_bat_smooth_algo_prepare(di);
			di->work_mode = MODE_SMOOTH;
		} else if ((rk816_bat_get_chrg_status(di) == CHARGE_FINISH) ||
			   rk816_bat_fake_finish_mode(di)) {
			//ESP_LOGD(TAG,"step2: change to finish mode...\n");
			rk816_bat_finish_algo_prepare(di);
			di->work_mode = MODE_FINISH;
		}
	} else {
		//ESP_LOGD(TAG,"step3: smooth algorithm...\n");
		rk816_bat_smooth_algorithm(di);
		if ((di->current_avg < 0) &&
		    (di->voltage_avg < di->pdata->zero_algorithm_vol)) {
			//ESP_LOGD(TAG,"step3: change to zero mode...\n");
			rk816_bat_zero_algo_prepare(di);
			di->work_mode = MODE_ZERO;
		} else if ((rk816_bat_get_chrg_status(di) == CHARGE_FINISH) ||
			   rk816_bat_fake_finish_mode(di)) {
			//ESP_LOGD(TAG,"step3: change to finish mode...\n");
			rk816_bat_finish_algo_prepare(di);
			di->work_mode = MODE_FINISH;
		}
	}
...
static void rk816_bat_relax_vol_calib(struct rk816_battery *di)
{
	...
	//ESP_LOGI(TAG,"sleep ocv calib: rsoc=%d, cap=%d\n", soc, cap);
}

static void rk816_bat_relife_age_flag(struct rk816_battery *di)
{
	uint8_t ocv_soc, ocv_cap, soc_level;
	...
	//ESP_LOGD(TAG,"<%s>. ocv_soc=%d, min=%lu, vol=%d\n", __func__,
	...
	if (!di->age_allow_update && ocv_soc <= 10) {
		...
		//ESP_LOGI(TAG,"resume: age_vol:%d, age_ocv_cap:%d, age_ocv_soc:%d, age_soc_level:%d, age_allow_update:%d, age_level:%d\n",
...
static int rk816_bat_sleep_dischrg(struct rk816_battery *di)
{
	...
	//ESP_LOGD(TAG,"<%s>. enter: dsoc=%d, rsoc=%d, rv=%d, v=%d, sleep_min=%lu\n",
	...
	if (di->dsoc <= di->rsoc) {
		...
		if (sleep_soc > 0) {
			//ESP_LOGI(TAG,"calib0: rl=%d, dl=%d, intval=%d\n",
		...
		//ESP_LOGD(TAG,"%s: dsoc<=rsoc, sum_cap=%d==>sleep_soc=%d, tgt_dsoc=%d\n",
		...
		//ESP_LOGI(TAG,"calib1: rsoc=%d, dsoc=%d, intval=%d\n",
			 di->rsoc, di->dsoc, sleep_soc);
		...
		//ESP_LOGD(TAG,"%s: dsoc>rsoc, sum_cap=%d=>sleep_soc=%d, gap_soc=%d\n",
	...
	if (di->voltage_avg <= pwroff_vol - 70) {
		di->dsoc = 0;
		rk_send_wakeup_key();
		//ESP_LOGI(TAG,"low power sleeping, shutdown... %d\n", di->dsoc);
	}

	if (ocv_soc_updated && sleep_soc && (di->rsoc - di->dsoc) < 5 &&
	    di->dsoc < 40) {
		di->dsoc--;
		//ESP_LOGI(TAG,"low power sleeping, reserved... %d\n", di->dsoc);
	}

	if (di->dsoc <= 0) {
		di->dsoc = 0;
		rk_send_wakeup_key();
		//ESP_LOGI(TAG,"sleep dsoc is %d...\n", di->dsoc);
	}

	//ESP_LOGD(TAG,"<%s>. out: dsoc=%d, rsoc=%d, sum_cap=%d\n",
...
static void rk816_bat_power_supply_changed(struct rk816_battery *di)
{
	...
	power_supply_changed(di->bat);
	//ESP_LOGI(TAG,"changed: dsoc=%d, rsoc=%d, v=%d, ov=%d c=%d, cap=%d, f=%d, st=%s, hotdie=%d\n",
		 di->dsoc, di->rsoc, di->voltage_avg, di->voltage_ocv,
		 di->current_avg, di->remain_cap, di->fcc, bat_status[status],
		 !!(thermal & HOTDIE_STS));

	//ESP_LOGI(TAG,"dl=%d, rl=%d, v=%d, halt=%d, halt_n=%d, max=%d, init=%d, sw=%d, calib=%d, below0=%d, force=%d\n",
...
static uint8_t rk816_bat_check_reboot(struct rk816_battery *di)
{
	uint8_t cnt;
...
static void rk816_bat_check_charger(struct rk816_battery *di)
{
	...
	if ((buf & PLUG_IN_STS) != 0 && !rk816_bat_chrg_online(di)) {
		...
		//ESP_LOGI(TAG,"pmic detect charger.. USB\n");
	/* pmic not detect plug in, but one of ac/usb/dc_in online, reset */
		...
		//ESP_LOGI(TAG,"pmic not detect charger..\n");
	}
}
...
static void rk816_bat_rsoc_daemon(struct rk816_battery *di)
{
	...
	if ((di->remain_cap < 0) && (di->fb_blank != 0)) {
		...
		//ESP_LOGD(TAG,"sec=%ld, hold_sec=%ld\n", sec, base2sec(sec));
		if (base2sec(sec) >= 60) {
			sec = 0;
			di->dbg_cap_low0++;
			...
			//ESP_LOGI(TAG,"adjust cap below 0 --> %d, rsoc=%d\n",
...
static void rk816_bat_update_info(struct rk816_battery *di)
{
	...
	if (di->remain_cap > di->fcc) {
		...
		//ESP_LOGD(TAG,"<%s>. cap: remain=%d, sm_remain=%d\n",
...
static void rk816_bat_init_dsoc_algorithm(struct rk816_battery *di)
{
	...
	if (rk816_bat_get_chrg_status(di) == CHARGE_FINISH) {
		if (di->algo_rest_mode == MODE_FINISH) {
			...
			if ((rest / DIV(soc_sec)) > 0) {
				if (di->dsoc < 100) {
					...
					//ESP_LOGI(TAG,"algorithm rest(%d) dsoc inc: %d\n",
						 rest, di->dsoc);
				} else {
					di->algo_rest_val = 0;
				}
		...
		/* discharge speed up */
			...
			if (di->dsoc > di->rsoc) {
				di->dsoc--;
				di->algo_rest_val = rest % 1000;
				//ESP_LOGI(TAG,"algorithm rest(%d) dsoc sub: %d\n",
					 rest, di->dsoc);
			} else {
				di->algo_rest_val = 0;
			}
	...
	//ESP_LOGD(TAG,"<%s>. init: org_rest=%d, rest=%d, mode=%s; "
...
static void rk816_bat_save_algo_rest(struct rk816_battery *di)
{
	...
	//ESP_LOGD(TAG,"<%s>. rest: algo=%d, mode=%s, last_rest=%d; zero=%d, chrg=%d, dischrg=%d, finish=%lu\n",
...
static int rk816_bat_get_ntc_res(struct rk816_battery *di)
{
	...
	//ESP_LOGD(TAG,"<%s>. val=%d, ntc_res=%d, factor=%d\n",
	    __func__, val, res, di->pdata->ntc_factor);

	//ESP_LOGD(TAG,"<%s>. t=[%d'C(%d) ~ %dC(%d)]\n", __func__,
...
static void rk816_bat_update_temperature(struct rk816_battery *di)
{
	...
	if (ntc_size) {
		...
		if (res < ntc_table[ntc_size - 1]) {
			//ESP_LOGI(TAG,"bat ntc upper max degree: R=%d\n", res);
		} else if (res > ntc_table[0]) {
			//ESP_LOGI(TAG,"bat ntc lower min degree: R=%d\n", res);
		} else {
...
static void rk816_bat_update_ocv_table(struct rk816_battery *di)
{
	...
	//ESP_LOGD(TAG,"<%s>: temp_idx=%d, curr_temp=%d, last_avg=%d, curr_avg=%d\n",
	...
	if (curr_avg_temp != last_avg_temp) {
		//ESP_LOGI(TAG,"OCV table update, temperature now=%d, last=%d\n",
...
static void rk816_bat_discnt_evt_worker(struct work_struct *work)
{
	...
	if (extcon_get_cable_state_(di->cable_edev, EXTCON_USB) == 0) {
		//ESP_LOGI(TAG,"receive extcon notifier event: DISCNT...\n");
...
static void rk816_bat_host_evt_worker(struct work_struct *work)
{
	...
	if (extcon_get_cable_state_(edev, EXTCON_USB_VBUS_EN) > 0) {
		...
		//ESP_LOGI(TAG,"receive extcon notifier event: OTG ON...\n");
		if (di->dc_in && di->pdata->power_dc2otg)
			//ESP_LOGI(TAG,"otg power from dc adapter\n");
		else
			rk816_bat_set_otg_power(di, USB_OTG_POWER_ON);
	} else if (extcon_get_cable_state_(edev, EXTCON_USB_VBUS_EN) == 0) {
		//ESP_LOGI(TAG,"receive extcon notifier event: OTG OFF...\n");
...
static void rk816_bat_charger_evt_worker(struct work_struct *work)
{
	...
	if (charger != USB_TYPE_UNKNOWN_CHARGER) {
		//ESP_LOGI(TAG,"receive extcon notifier event: %s...\n",
			 event[charger]);
...
static irqreturn_t rk816_vb_low_irq(int irq, void *bat)
{
	...
	//ESP_LOGI(TAG,"lower power yet, power off system! v=%d\n",
		 di->voltage_avg);
	di->dsoc = 0;
	rk_send_wakeup_key();
	power_supply_changed(di->bat);
...
static void rk816_bat_init_sysfs(struct rk816_battery *di)
{
	...
	for (i = 0; i < ARRAY_SIZE(rk816_bat_attr); i++) {
		...
		if (ret)
			//ESP_LOGE(TAG, "create bat node(%s) error\n",
...
static int rk816_bat_init_irqs(struct rk816_battery *di)
{
	...
	ret = devm_request_threaded_irq(di->dev, vb_lo_irq, NULL,
					...
					"rk816_vb_low", di);
	if (ret) {
		//ESP_LOGE(TAG, "vb low irq request failed!\n");
		return ret;
	}
	...
	ret = devm_request_threaded_irq(di->dev, plug_in_irq, NULL,
					...
					"rk816_plug_in", di);
	if (ret) {
		//ESP_LOGE(TAG, "plug in irq request failed!\n");
		return ret;
	}
	...
	ret = devm_request_threaded_irq(di->dev, plug_out_irq, NULL,
					...
					"rk816_plug_out", di);
	if (ret) {
		//ESP_LOGE(TAG, "plug out irq request failed!\n");
		return ret;
	}
	...
	ret = devm_request_threaded_irq(di->dev, cvtlmt_irq, NULL,
					...
					"rk816_cvtlmt", di);
	if (ret) {
		//ESP_LOGE(TAG, "cvtlmt irq request failed!\n");
		return ret;
	}
...
static enum charger_t rk816_bat_init_gpio_dc_det(struct rk816_battery *di)
{
	...
	if (gpio_is_valid(di->pdata->dc_det_pin)) {
		ret = devm_gpio_request(di->dev, di->pdata->dc_det_pin,
					"rk816_dc_det");
		if (ret < 0) {
			//ESP_LOGE(TAG, "Failed to request gpio %d\n",
				di->pdata->dc_det_pin);
			goto out;
		}
		...
		if (ret) {
			//ESP_LOGE(TAG, "failed to set gpio input\n");
			goto out;
		}
		...
		if (ret != 0) {
			//ESP_LOGE(TAG, "rk816_dc_det_irq request failed!\n");
			goto out;
		}
...
static int rk816_bat_init_charger(struct rk816_battery *di)
{
	...
	//ESP_LOGI(TAG,"register extcon evt notifier\n");
...
static void rk816_bat_init_fg(struct rk816_battery *di)
{
	...
	//ESP_LOGD(TAG,"nac=%d cap=%d ov=%d v=%d rv=%d dl=%d rl=%d c=%d\n",
				     ...
				     struct device_node *np)
{
	...
	if (of_find_property(np, "table_t0", &length) &&
		...
		//ESP_LOGD(TAG,"%s: read table_t0\n", __func__);

		if (of_property_read_uint32_t_array(np, "table_t0",
					       pdata->table_t[idx],
					       pdata->ocv_size)) {
			//ESP_LOGE(TAG, "invalid table_t0\n");
			return -EINVAL;
		}

		if (of_property_read_uint32_t_index(np, "temp_t0", 1, &value) ||
		    of_property_read_uint32_t_index(np, "temp_t0", 0, &negative)) {
			//ESP_LOGE(TAG, "invalid temp_t0\n");
			return -EINVAL;
		}
		if (negative)
	...
	if (of_find_property(np, "table_t1", &length) &&
		...
		//ESP_LOGD(TAG,"%s: read table_t1\n", __func__);

		if (of_property_read_uint32_t_array(np, "table_t1",
					       pdata->table_t[idx],
					       pdata->ocv_size)) {
			//ESP_LOGE(TAG, "invalid table_t1\n");
			return -EINVAL;
		}

		if (of_property_read_uint32_t_index(np, "temp_t1", 1, &value) ||
		    of_property_read_uint32_t_index(np, "temp_t1", 0, &negative)) {
			//ESP_LOGE(TAG, "invalid temp_t1\n");
			return -EINVAL;
		}
		if (negative)
			pdata->temp_t[idx] = -value;
	...
	if (of_find_property(np, "table_t2", &length) &&
		...
		//ESP_LOGD(TAG,"%s: read table_t2\n", __func__);

		if (of_property_read_uint32_t_array(np, "table_t2",
					       pdata->table_t[idx],
					       pdata->ocv_size)) {
			//ESP_LOGE(TAG, "invalid table_t2\n");
			return -EINVAL;
		}

		if (of_property_read_uint32_t_index(np, "temp_t2", 1, &value) ||
		    of_property_read_uint32_t_index(np, "temp_t2", 0, &negative)) {
			//ESP_LOGE(TAG, "invalid temp_t2\n");
			return -EINVAL;
		}
		if (negative)
			pdata->temp_t[idx] = -value;
	...
	if (of_find_property(np, "table_t3", &length) &&
		...
		//ESP_LOGD(TAG,"%s: read table_t3\n", __func__);

		if (of_property_read_uint32_t_array(np, "table_t3",
					       pdata->table_t[idx],
					       pdata->ocv_size)) {
			//ESP_LOGE(TAG, "invalid table_t3\n");
			return -EINVAL;
		}

		if (of_property_read_uint32_t_index(np, "temp_t3", 1, &value) ||
		    of_property_read_uint32_t_index(np, "temp_t3", 0, &negative)) {
			//ESP_LOGE(TAG, "invalid temp_t3\n");
			return -EINVAL;
		}
		if (negative)
			pdata->temp_t[idx] = -value;
	...
	//ESP_LOGD(TAG,"realtime ocv table nums=%d\n", di->pdata->temp_t_num);

	if (dbg_enable) {
		for (j = 0; j < pdata->temp_t_num; j++) {
			//ESP_LOGD(TAG,"\n\ntemperature[%d]=%d\n", j, pdata->temp_t[j]);
			for (i = 0; i < di->pdata->ocv_size; i++)
				//ESP_LOGD(TAG,"table_t%d[%d]=%d\n",
...
static int rk816_bat_parse_dt(struct rk816_battery *di)
{
	...
	pdata->ocv_size = length / sizeof(uint32_t);
	if (pdata->ocv_size <= 0) {
	...
	ret = of_property_read_uint32_t_array(np, "ocv_table", pdata->ocv_table,
					 pdata->ocv_size);
	if (ret < 0)
		return ret;
	...
	ret = of_property_read_uint32_t(np, "design_capacity", &out_value);
	if (ret < 0) {
	...
	ret = of_property_read_uint32_t(np, "design_qmax", &out_value);
	if (ret < 0) {
	...
	ret = of_property_read_uint32_t(np, "max_chrg_current", &out_value);
	if (ret < 0) {
	...
	ret = of_property_read_uint32_t(np, "max_input_current", &out_value);
	if (ret < 0) {
	...
	ret = of_property_read_uint32_t(np, "max_chrg_voltage", &out_value);
	if (ret < 0) {
	...
	of_property_read_uint32_t(np, "sample_res", &pdata->sample_res);

	if (!of_find_property(np, "lp_input_current", &length)) {
		pdata->lp_input_current = 0;
	} else {
		of_property_read_uint32_t_index(np, "lp_input_current", 0,
					   &pdata->lp_input_current);
		of_property_read_uint32_t_index(np, "lp_input_current", 1,
					   &pdata->lp_soc_min);
		of_property_read_uint32_t_index(np, "lp_input_current", 2,
					   &pdata->lp_soc_max);
	...
	ret = of_property_read_uint32_t(np, "fb_temperature", &pdata->fb_temp);
	if (ret < 0)
		dev_err(dev, "fb_temperature missing!\n");

	ret = of_property_read_uint32_t(np, "energy_mode", &pdata->energy_mode);
	if (ret < 0)
		dev_err(dev, "energy_mode missing!\n");

	ret = of_property_read_uint32_t(np, "max_soc_offset",
				   &pdata->max_soc_offset);
	if (ret < 0)
		dev_err(dev, "max_soc_offset missing!\n");

	ret = of_property_read_uint32_t(np, "monitor_sec", &pdata->monitor_sec);
	if (ret < 0)
		dev_err(dev, "monitor_sec missing!\n");

	ret = of_property_read_uint32_t(np, "zero_algorithm_vol",
				   &pdata->zero_algorithm_vol);
	if (ret < 0)
		dev_err(dev, "zero_algorithm_vol missing!\n");

	ret = of_property_read_uint32_t(np, "zero_reserve_dsoc",
				   &pdata->zero_reserve_dsoc);

	ret = of_property_read_uint32_t(np, "virtual_power", &pdata->bat_mode);
	if (ret < 0)
		dev_err(dev, "virtual_power missing!\n");

	ret = of_property_read_uint32_t(np, "power_dc2otg", &pdata->power_dc2otg);
	if (ret < 0)
		dev_err(dev, "power_dc2otg missing!\n");

	ret = of_property_read_uint32_t(np, "bat_res", &pdata->bat_res);
	if (ret < 0)
		dev_err(dev, "bat_res missing!\n");

	ret = of_property_read_uint32_t(np, "sleep_enter_current",
				   &pdata->sleep_enter_current);
	if (ret < 0)
		dev_err(dev, "sleep_enter_current missing!\n");

	ret = of_property_read_uint32_t(np, "sleep_exit_current",
				   &pdata->sleep_exit_current);
	if (ret < 0)
		dev_err(dev, "sleep_exit_current missing!\n");

	ret = of_property_read_uint32_t(np, "sleep_filter_current",
				   &pdata->sleep_filter_current);
	if (ret < 0)
		dev_err(dev, "sleep_filter_current missing!\n");

	ret = of_property_read_uint32_t(np, "power_off_thresd", &pdata->pwroff_vol);
	if (ret < 0)
		dev_err(dev, "power_off_thresd missing!\n");

	ret = of_property_read_uint32_t(np, "otg5v_suspend_enable",
	...
	if (!of_find_property(np, "dc_det_gpio", &length)) {
		pdata->dc_det_pin = -1;
		of_property_read_uint32_t(np, "dc_det_adc", &pdata->dc_det_adc);
		if (!pdata->dc_det_adc)
			//ESP_LOGI(TAG,"not support dc\n");
		else
			//ESP_LOGI(TAG,"support adc dc\n");
	} else {
		//ESP_LOGI(TAG,"support gpio dc\n");
	...
	if (!of_find_property(np, "ntc_table", &length)) {
		...
		ret = of_property_read_uint32_t_index(np, "ntc_degree_from", 1,
		...
		of_property_read_uint32_t_index(np, "ntc_degree_from", 0,
					   &out_value);
		if (out_value)
			pdata->ntc_degree_from = -pdata->ntc_degree_from;

		pdata->ntc_size = length / sizeof(uint32_t);
	}
	...
	if (pdata->ntc_size) {
		...
		ret = of_property_read_uint32_t_array(np, "ntc_table",
						 pdata->ntc_table,
						 pdata->ntc_size);
		if (ret < 0)
			return ret;
	...
	// //ESP_LOGD(TAG,"the battery dts info dump:\n"
	//     "bat_res:%d\n"
	//     "res_sample:%d\n"
				 ...
				 pm_message_t state)
{
	...
	if (di->pdata->otg5v_suspend_enable) {
		if ((di->otg_in && !di->dc_in) ||
			...
			//ESP_LOGI(TAG,"suspend: otg 5v on\n");
		} else {
			/* disable sleep otg5v */
			rk816_bat_set_bits(di, RK816_SLP_DCDC_EN_REG,
					   OTG_BOOST_SLP_ON, 0);
			//ESP_LOGI(TAG,"suspend: otg 5v off\n");
		}
	} else {
		/* disable sleep otg5v */
		rk816_bat_set_bits(di, RK816_SLP_DCDC_EN_REG,
	...
	if ((di->work_mode == MODE_ZERO) &&
		...
		//ESP_LOGD(TAG,"suspend: MODE_ZERO exit...\n");
	...
	//ESP_LOGI(TAG,"suspend: dl=%d rl=%d c=%d v=%d cap=%d at=%ld ch=%d st=%s\n",
...
static int rk816_battery_resume(void)
{
	int interval_sec, pwroff_vol, time_step = DISCHRG_TIME_STEP1;
	struct rk816_battery *di = NULL;
	uint8_t st;
	...
	//ESP_LOGI(TAG,"resume: dl=%d rl=%d c=%d v=%d rv=%d cap=%d dt=%d at=%ld ch=%d st=%s\n",
